...
{    
    ...
    vec3 result = CalcDirLight(dirLight, norm, viewDir, material_color.rgb);
    // phase 2: point lights
    for(int i = 0; i < NR_POINT_LIGHTS; i++)
        result += CalcPointLight(pointLights[i], norm, FragPos, viewDir, material_color.rgb);    
    // phase 3: spot light
    result += CalcSpotLight(spotLight, norm, FragPos, viewDir, material_color.rgb);    
...
void use_lighting(vec4 vertex, vec3 vertex_normal, vec4 raw_color){
    ...
    // vec4 light_intensity;
     ...
     for(int i = 0; i < number_point_lights; i++)// !!!!!!!!
        result += CalcPointLight(pointLights[i], vertex_normal, vec3(vertex.x,vertex.y,vertex.z), viewDir, material); // !!!!!!!!
	 ...
	 for(int i = 0; i < number_spot_lights; i++)// !!!!!!!!
		result += CalcSpotLight(spotLight[i], vertex_normal, vec3(vertex.x,vertex.y,vertex.z), viewDir, material); // !!!!!!!!
		//vec3 CalcSpotLight(SpotLight light, vec3 normal, vec3 fragPos, vec3 viewDir,Material material)
		//result += CalcSpotLight(spotLight, vertex_normal, FragPos, viewDir, material); // !!!!!!!!
