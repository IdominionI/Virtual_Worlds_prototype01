...
class voxel_hcp_scene_objects_class : public scene_objects_category_class, public std::vector<voxel_hcp_object_class*> {
...
public:
	...
	//void  *get_entity_object(object_id_type object_id) {
	object_basis_class *get_entity_object(object_id_type object_id) {
		int index = get_object_index(object_id);
//printf("voxel_hcp_scene_objects2_class :: get_voxel_hcp_object %i\n", index);
...
//printf("voxel_hcp_scene_objects2_class : add_new_object 000: %i \n", object_id);
...
//printf("voxel_hcp_scene_objects2_class : add_new_object 111: %i \n", at(size() - 1)->object_id);
		return true;
	}
	...
	void delete_objects() {
		for (int i = 0; i < size(); i++) {
			voxel_hcp_object_class* voxel_hcp_object_to_delete = at(i);
			delete voxel_hcp_object_to_delete;
		}
		clear();
	}
