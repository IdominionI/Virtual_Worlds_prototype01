...
public:
	...
	point_cloud_data_class *voxel_hcp_cloud = NULL;

	std::string shader_basis_code_directory_pathname = "Resources/Shaders/Default/Voxel/Shader_basis_code/";

	std::string default_shader_file_directory   = "Resources/Shaders/Default/Voxel/";
	std::string shader_file_directory           = "Resources/Shaders/Default/Voxel/";
	...
	bool define_voxel_hcp_render_object(scene_node_class <render_object_class> *entity_render_object,voxel_hcp_object_class *voxel_hcp_object) {
		voxel_hcp_cloud = new point_cloud_data_class;
	...
	bool update_geometry_vertex_cloud_data(point_cloud_data_class *voxel_hcp_cloud,scene_node_class <render_object_class> *entity_render_object, log_panel_class *log_panel) {
		if (voxel_hcp_cloud == NULL) {
		...
		point_cloud_data_class *point_cloud_data = static_cast<point_cloud_data_class*>(entity_render_object->scene_graph_object.scene_object_class.geometry);
	...
	bool define_shader_program(scene_node_class <render_object_class> *entity_render_object, log_panel_class *log_panel) {
		...
		if (shader_material->use_point_shader) {
			...
			if (!vw_point_geometry_shader.generate_point_geometry_shader_code()) {
				if (log_panel != NULL) log_panel->application_log.AddLog("ERROR :  Vertex shader file code failed. Unable to generate the point geometry shader source code");
				return false;
			}
		}
		else {
			...
			if (!vw_geometry_shader.generate_geometry_shader_code()) {
				if (log_panel != NULL) log_panel->application_log.AddLog("ERROR :  Vertex shader file code failed. Unable to generate the geometry shader source code");
				return false;
			}
		}
		...
		if (!vw_fragment_shader.generate_fragment_shader_code()) {
			if (log_panel != NULL) log_panel->application_log.AddLog("ERROR :  Vertex shader file code failed. Unable to generate the fragment shader source code");
			return false;
		}
...
private:
	...
	bool define_initial_shader_program(scene_node_class <render_object_class>* entity_render_object,voxel_hcp_object_class *voxel_hcp_object) {
		shader_component_struct_type* shader_material = &entity_render_object->scene_graph_object.scene_object_class.shader_material;
