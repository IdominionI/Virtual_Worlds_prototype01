...
#include "hex_animation_functions.h"
#include "../Render/hex_surface_render.h"

class scene_hex_surface_animation_class : public animation_base_class {
public:
...
//printf("scene_hsp_voxel_animation_class::define_objects_to_execute  22 :: hex_surface_animation_functions.define_hex_surface_objects_to_execute\n", current_selected_object_type_id, current_selected_object_id, animation_selection);
	...
	void define_initial_frame_paramters(bool use_textures, bool use_automata, int max_automata_step_value, int frame_step_end) {
		hex_surface_animation_functions.define_initial_frame_paramters(use_textures, use_automata, max_automata_step_value, frame_step_end);
	}
	...
	//printf("animation :: perform_animation_frame 00AA scene_manager != NULL\n");
		...
		for (int i = 0; i < hex_surface_animation_functions.hex_surface_objects_to_execute.size(); i++) {
			hex_surface_object_class* voxel_hcp_object = hex_surface_animation_functions.hex_surface_objects_to_execute[i];
...
//printf("restore_initial_frame 44 \n");
	...
	bool export_geometry(int animation_surface, std::string saved_mesh_pathname, int frame, std::string file_format) {
		bool export_hex_surface_mesh = false;
...
//printf("scene_hsp_voxel_animation_class:export_geometry animation_surface == 0\n");
			export_hex_surface_mesh = hex_surface_animation_functions.export_hex_surface_center_point_data(saved_mesh_pathname, frame, file_format);
		}
		else
...
//printf("scene_hsp_voxel_animation_class:export_geometry animation_surface == 2\n");
					export_hex_surface_mesh = hex_surface_animation_functions.export_hex_surface_face_surface_data(saved_mesh_pathname, frame, file_format);
				}
			}

		if (!export_hex_surface_mesh)
			return false;
		else
			return true;
	}
