...
//}
	...
	bool add_hex_surface_vbo_vertex(size_t index) {
		...
		index_vector3 iv = hex_surface_object_data.get_matrix_coordinate(index);
...
//stream << "3 " << n - x_dim << " " << n << " " << n - 1 << endl;
						if (i < x_dim - 1) {
							...
							vertex_1 = point_cloud.vertices[n].mPos;
							vertex_2 = point_cloud.vertices[n - x_dim].mPos;

							glm::vec3 normal = calculate_normal_vector(vertex_0,vertex_1,vertex_2);

							point_cloud.vertices[n - x_dim+1].mNormal = normal;
							point_cloud.vertices[n].mNormal           = normal;
							point_cloud.vertices[n - x_dim].mNormal   = normal;
...
//stream << "3 " << n - x_dim + 1 << " " << n << " " << n - x_dim << endl;
	...
	bool define_vbo_face_normals() {
		...
		for (int i = 0; i < number_triangles; i++) {
			int offset = i * 3;
			glm::vec3 vertex_0 = point_cloud.vertices[offset].mPos;
			glm::vec3 vertex_1 = point_cloud.vertices[offset + 1].mPos;
			glm::vec3 vertex_2 = point_cloud.vertices[offset + 2].mPos;

			glm::vec3 normal = calculate_normal_vector(vertex_0, vertex_1, vertex_2);

			point_cloud.vertices[offset].mNormal = normal;
			point_cloud.vertices[offset + 1].mNormal = normal;
			point_cloud.vertices[offset + 2].mNormal = normal;
		}
