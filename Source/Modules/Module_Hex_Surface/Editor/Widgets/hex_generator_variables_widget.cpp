...
void hex_surface_generator_variables_widget_class::variables_manager(hex_surface_generator_parameters_struct_type& hex_surface_generator_parameters) {
	...
	hex_surface_generator_parameter_variable_struct_type* real_variable = NULL;
	hex_surface_generator_parameter_int_variable_struct_type* int_variable = NULL;
	hex_surface_generator_parameter_bool_variable_struct_type* bool_variable = NULL;
	...
	if (variables_open) {
		...
		if (real_var_open) {
			...
			for (int j = 0; j < hex_surface_generator_parameters.variables.size(); j++) {
				real_variable = &hex_surface_generator_parameters.variables[j]; // Note :: Will not work uinless real_variable is a pointer to the entry of structure data to modify
		...
		if (int_var_open) {
			...
			for (int j = 0; j < hex_surface_generator_parameters.int_variables.size(); j++) {
				int_variable = &hex_surface_generator_parameters.int_variables[j]; // Note :: Will not work uinless real_variable is a pointer to the entry of structure data to modify
		...
		if (bool_var_open) {
			...
			for (int j = 0; j < hex_surface_generator_parameters.bool_variables.size(); j++) {
				bool_variable = &hex_surface_generator_parameters.bool_variables[j]; // Note :: Will not work uinless real_variable is a pointer to the entry of structure data to modify
		...
		if (current_selected_var_type_id > -1 && current_selected_var_id > -1) {
			...
			if (ImGui::BeginPopupContextWindow("gr1", 1, true)) {
				if (ImGui::MenuItem("Delete Variable2")) delete_variable(current_selected_var_type_id, current_selected_var_id, hex_surface_generator_parameters);
				ImGui::EndPopup();// Root
			}
		}
		else {
			if (current_selected_var_type_id > -1 && current_selected_var_id < 0)
				if (ImGui::BeginPopupContextWindow("gr2", 1, true)) {
					if (ImGui::MenuItem("Add Variable")) add_variable(current_selected_var_type_id, hex_surface_generator_parameters);
					ImGui::EndPopup();// Root
				}
		}
		// printf("real var\n");
...
void hex_surface_generator_variables_widget_class::add_variable(int var_type_id, hex_surface_generator_parameters_struct_type& hex_surface_generator_parameters) {
	printf("in add variabel %i\n", var_type_id);

	// Switch cannot use var_type_id so need to rvert to if statements !!!!!!

	if (var_type_id == real_var_type_id) {
		hex_surface_generator_parameter_variable_struct_type real_var;
		real_var.variable_name += std::to_string(hex_surface_generator_parameters.variables.size());
		hex_surface_generator_parameters.variables.push_back(real_var);
		return;
	}

	if (var_type_id == int_var_type_id) {
		hex_surface_generator_parameter_int_variable_struct_type int_var;
		int_var.variable_name += std::to_string(hex_surface_generator_parameters.int_variables.size());

		hex_surface_generator_parameters.int_variables.push_back(int_var);
		return;
	}

	if (var_type_id == bool_var_type_id) {
		hex_surface_generator_parameter_bool_variable_struct_type bool_var;
		bool_var.variable_name += std::to_string(hex_surface_generator_parameters.bool_variables.size());

		hex_surface_generator_parameters.bool_variables.push_back(bool_var);
		return;
	}
}

void hex_surface_generator_variables_widget_class::delete_variable(int var_type_id, int var_id, hex_surface_generator_parameters_struct_type& hex_surface_generator_parameters) {
	printf("in delete variabel %i : %i\n", var_type_id, var_id);

	if (var_type_id == real_var_type_id) {
		hex_surface_generator_parameters.variables.erase(hex_surface_generator_parameters.variables.begin() + var_id);
		return;
	}

	if (var_type_id == int_var_type_id) {
		hex_surface_generator_parameters.int_variables.erase(hex_surface_generator_parameters.int_variables.begin() + var_id);
		return;
	}

	if (var_type_id == bool_var_type_id) {
		hex_surface_generator_parameters.bool_variables.erase(hex_surface_generator_parameters.bool_variables.begin() + var_id);
		return;
	}
