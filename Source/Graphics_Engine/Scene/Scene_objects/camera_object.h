...
{
public:
	glm::vec3 mPosition = { 0.0f, 0.0f, 15.0f };
...
//printf("camera update : % f : %f :%f \n", mPosition.x, mPosition.y, mPosition.z);
	...
	void move_forward() {
		...
		if (orbital_rotation) set_orbit_distance();
		update_view_matrix();
	}
...
//printf("on_mouse_move button middle\n");
	...
	void update_view_matrix() {
		if (orbital_rotation) {
			// There is problem with using glm::distance that seems to change the value of 
			// mDistance even though the mouse is not in motion and no movement is made
			// of the camera location with the keypad. Needed to resort to calculating
			// distance only when selecting orbit motion and moving camera froward or
			// backward during orbit motion, which is more efficient anyway.
