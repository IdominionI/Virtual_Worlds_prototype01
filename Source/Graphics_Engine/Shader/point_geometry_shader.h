...
/*
	vw_point_geometry_shader_class

	C++ class to create code to define an OpenGL glsl point geometry shader program from source text files.
	...
	compilation
*/
class vw_point_geometry_shader_class : public vw_shader_basis_class {
public:
	bool generate_point_geometry_shader_code() {
		import_compute_expression_class import_compute_expression;// Class to import cutom user glsl code

		import_compute_expression.filename_to_read = shader_parameters.point_shader_file_pathname;// Define file pathname of custom user glsl code file to import

		if (!import_compute_expression.import_compute_expression()) {
			//QMessageBox::information(0, "Function Expression Success", "vw_point_geometry_shader_class : generate_point_geometry_shader_code: import user voxel geometry shader code of file " + shader_parameters.geometry_shader_file_pathname + " failed.", QMessageBox::Ok); // testing only
			printf("vw_point_geometry_shader_class : generate_point_geometry_shader_code: import user voxel geometry shader code of file %s failed.", shader_parameters.point_shader_file_pathname);
			return false;
		}
		...
		create_point_geometry_shader_source_code(); // Colate all OpenGL glsl code sections into one glgl source code program
...
//QMessageBox::information(0, "Function Expression Success", "vw_vertex_shader_class : generate_vertex_shader_code() 006", QMessageBox::Ok); // testing only
	...
	void create_point_geometry_shader_source_code() {
		shader_code = "";
...
private:
	...
	void define_reserved_uniforms() {
		reserved_uniforms_s = import_code(shader_basis_code_directory_pathname + "pgs_reserved_uniforms.txt");
		/*
		...
		//layout(triangle_strip, max_vertices = 48) out; // This required to display a voxel of 12 faces and 4 vertices per face
			reserved_uniforms_s += "layout(points, max_vertices = 1) out;    // This required to display a voxel center of 1 point\n";
	...
	void define_function_s() {
		functions_s = import_code(shader_basis_code_directory_pathname + "pgs_defined_functions.txt");
		//functions_s = "// ******************* No in Built functions defined at this time *********************\n";
	}


	void define_main() {
		main_s = import_code(shader_basis_code_directory_pathname + "pgs_main.txt");
		//main_s = "void main() {\n";
	}

	void define_ouput() {
		output_s = import_code(shader_basis_code_directory_pathname + "pgs_output.txt");
		//output_s = "}\n";
	}
